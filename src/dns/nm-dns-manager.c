...
#define STUB_RESOLV_CONF           NMRUNDIR "/stub-resolv.conf"

static void
update_resolv_conf_stub (NMDnsManager *self,
                         char **searches,
                         char **nameservers,
                         char **options)
{
	...
	if (!g_file_set_contents (STUB_RESOLV_CONF,
	                          content,
	                          -1,
	                          &local)) {
		_LOGD ("update-resolv-stub: failure to write stub resolv.conf: %s",
		       local->message);
		g_error_free (local);
		return;
	}

	_LOGT ("update-resolv-stub: stub '%s' successfully written",
	       STUB_RESOLV_CONF);
}
            ...
            GError **error)
{
	...
	update_resolv_conf_stub (self, searches, nameservers, options);
